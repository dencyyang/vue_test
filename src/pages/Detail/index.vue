...
export default {
  ...
  computed: {
    ...mapState({
      price: (state) => {
        return state.detail.goodInfo.price;
      },
    }),
    ...mapGetters([
      "categoryView",
  ...
  methods: {
    ...
    changeActive(saleAttrValue, attr) {
      ...
      return (saleAttrValue.isChecked = 1);
    },
    //为input事件，输入框数据修改时调用事件回调
    changeSkuNum(event) {
      ...
      if (isNaN(value) || value < 1) {
        ...
        //合法输入取整
        this.skuNum = prase.value;
      }
    },
    //添加到购物车
    //1.发送请求，将产品添加到数据库
    /* 当前这里派发一个actions，向服务器发送请求，判断加入购物车是否成功，进行响应的操作
     this.$store.dispatch('addOrUpdateShopCart',{skuId:this.$route.params.skuid,skuNum:this.skuNum})
     这行代码调用了vuex仓库中的addOrUpdateShopCart，这个方法加上了async返回的一定是一个Promise
     则要么成功要么失败 */
    //2.需要知道请求是否成功，成功则进行路由跳转，否则给用户错误提示
    //3.失败给用户提示
    async addShopCart() {
      try {
        //成功的捕获
        ...
        //路由跳转并把产品信息携带给下一级路由组件
        //一些简单的数据通过query形式传递给路由组件
        //产品信息的数据【比较复杂的：skuInfo】通过会话存储(不会持久化，会话结束数据消失)
        //方法一(不美观)
       /*  this.$router.push({
          name: "addcartsuccess",
          path: "/AddCartSuccess",
